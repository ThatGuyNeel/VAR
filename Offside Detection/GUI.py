...
class VideoWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Offside Detection")
    ...
    def capture_frame(self):
        ...
        # First try millisecond seeking (more accurate)
        cap.set(cv2.CAP_PROP_POS_MSEC, pos_ms)
        ret, frame1 = cap.read()

        if not ret:
            # Fallback 1: Try seeking by frame number if millisecond seek fails
            total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            frame_number = int((pos_ms / 1000.0) * fps)
        ...
        if not ret:
            # Fallback 2: One more attempt with position() if position seek fails
            cap2 = cv2.VideoCapture(self.current_filepath)
            if cap2.isOpened():
                cap2.set(cv2.CAP_PROP_POS_MSEC, pos_ms)
                ret, frame1 = cap2.read()
                if not ret:
                    # Final fallback: try frame-based positioning again
                    frame_number = int((pos_ms / 1000.0) * fps)
