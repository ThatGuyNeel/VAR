...
class Visualizer:
    ...
    def draw_stats(frame, stats, frame_num):
        """Draw statistics overlay with relative sizing"""
        h, w = frame.shape[:2]

        # Relative panel dimensions (8% of width, 10% of height)
        panel_width = int(w * 0.08)
        panel_height = int(h * 0.10)
        ...
        # Frame number
        cv2.putText(frame, f"Frame: {frame_num}", (10, int(25 * font_scale)), font, font_scale, (255, 255, 255), 2)

        # Active tracks
        cv2.putText(frame, f"Active: {stats['active_tracks']}", (10, int(50 * font_scale)), font, font_scale, (0, 255, 0), 2)

        # Total tracks
        cv2.putText(frame, f"Total: {stats['total_tracks']}", (10, int(75 * font_scale)), font, font_scale, (255, 255, 255), 2)

        # Class breakdown
        class_info = " | ".join([f"{CLASS_NAMES.get(k, k)}: {v}" for k, v in stats['class_counts'].items()])
        cv2.putText(frame, class_info, (10, int(100 * font_scale)), font, font_scale * 0.8, (200, 200, 200), 1)
